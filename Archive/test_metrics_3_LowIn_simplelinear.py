...
model_folder_name = r"3x3_3_LowIn_simple_3by3_01_07_03_run_arrival_change_201_011_simple_linear_t_2025-06-18_13-07-48"
trainstep = 1100
...
order_latest_stage_policy = DefaultPolicyMaker(
    "order_latest_stage", environment_name
).policy

# test all policies
used_seeds, results_rl, env_actions = tester100.test_rl_model_nobatch(rl_module=rl_mod)
used_seeds_hybrid, results_hybrid = tester100.test_hybrid_nobatch(
    rl_module=rl_mod, policy=latest_stage_policy
)

used_seeds_random, results_random = tester100.test_own_policy(policy=random_policy)
used_seeds_advanced_random, results_advanced_random = tester100.test_own_policy(
    policy=advanced_random_policy
)
used_seeds_latest_stage, results_latest_stage = tester100.test_own_policy(
    policy=latest_stage_policy
)
used_seeds_order_latest_stage, results_order_latest_stage = tester100.test_own_policy(
    policy=order_latest_stage_policy
)
